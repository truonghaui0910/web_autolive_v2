...
export default function PricingTable({ canRegister, limit, viewAllHref, sectionClassName = 'pb-20 pt-24 sm:pt-32' }) {
    const [active, setActive] = useState('livestream');
    const current = groups[active];
    const plans = limit ? current.plans.slice(0, limit) : current.plans;

    return (
        <section id="bang-gia" className={`container-lg ${sectionClassName}`}>
                ...
                {plans.map((plan, index) => (
                    ...
                    >
                        <GlowBorder
                            ...
                            beamDelay={(index * 10) / plans.length}
                        >
                            {current.type === 'tiered' ? (
            ...
            {viewAllHref && (
                <Reveal as="div" delay={0.15} className="mt-12 text-center">
                    <Link
                        href={viewAllHref}
                        className="inline-block rounded-lg border border-white/20 bg-gradient-to-b from-[var(--accent-purple)]/0 to-[var(--accent-purple)]/40 px-6 py-2.5 text-sm font-medium text-[#f4f0ff] transition hover:to-[var(--accent-purple)]/60"
                    >
